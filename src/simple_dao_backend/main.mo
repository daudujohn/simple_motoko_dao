...
shared actor class DAO(recievedInit : ?Types.BasicDaoStableStorage) = Self {
  var init: Types.BasicDaoStableStorage = switch (recievedInit) {
    case(?recieved) {
      recieved;
    };
    case (null) {
      var init_accounts: [Types.Account] = [];
    var init_proposals: [Types.Proposal] = [];
    var init_system_params: Types.SystemParams = {
      transfer_fee = {amount_e8s = 10_000};
      proposal_vote_threshold = {amount_e8s = 100_000};
      proposal_submission_deposit = {amount_e8s = 10_000};
    };
      {accounts = init_accounts; proposals =  init_proposals; system_params = init_system_params} : Types.BasicDaoStableStorage
    }
  };
